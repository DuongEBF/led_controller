...
#define UNINIT_HALF_PERIOD_MS   100   // -> 50% duty, 600 ms full period
#define STANDBY_PERIOD_MS       1550
#define STANDBY_ON_MS           100
#define ACTIVE_HALF_PERIOD_MS   500   // -> alternate every 300 ms
#define CRITICAL_HALF_PERIOD_MS 300   // -> 50% duty, 400 ms full period
/* USER CODE END PD */
/* ---------- Helpers ---------- */
...
static void update_leds_by_mav_state(uint8_t state, uint32_t now)
{
    ...
    set_pin_if_changed(gpio, LED_FRONT_GEAR_GREEN_Pin, 0, &last_front_green);
    set_pin_if_changed(gpio, LED_FRONT_GEAR_RED_Pin,   1, &last_front_red);
